    ...
    switch($methodeType) {
      case 'get':
        if($aantalArgumenten !== 0) {
          throw new ERROR\FrameworkException("$methodeNaam methode accepteert geen argumenten, zeker niet $aantalArgumenten argument(en).");
        }
        return $this->$eigenschap;

      case 'set':
        if($aantalArgumenten !== 1) {
          throw new ERROR\FrameworkException("$methodeNaam methode accepteert niet $aantalArgumenten argument(en).");
        }
